...
以下のいずれかに該当したら、**即座に/compact**を実行せよ：
...
2. /compactを再実行
3. それでも失敗したら/clearでリセット
```

**注意**: /clearは作業中の情報が失われるため、最終手段とすること。

## 🔴 Task tool活用でコンテキストを分散せよ

### Task tool使用の判断基準

以下のいずれかに該当する場合、**必ずTask toolを使用**してサブエージェントを起動せよ：
...
| ファイル読み取り | 10個以上見込み |
| コード分析 | 大規模（500行以上） |
| ドキュメント作成 | 複数セクション（5以上） |
| 並列処理が必要 | 独立したサブタスクが3つ以上 |
...
```
Task toolを使用して、専用のサブエージェントを起動する。
...
- subagent_type: "general-purpose"（調査・分析）または"planner"（設計・計画）
- description: 具体的な作業内容
- prompt: 詳細な指示書

【例】
description: "Google Apps ScriptのGmail連携方法をリサーチ"
prompt: |
  公式ドキュメントを参照し、GASでのGmail操作方法を調査せよ。
  以下の項目を含むmarkdownファイルを作成せよ：
  - GmailAppの主要メソッド
  - メール取得・送信のコード例
  - ベストプラクティス
```
...
1. サブエージェントの出力を確認
2. 必要な情報を抽出・整理
3. 自分のコンテキストには「結果のみ」を取り込む
4. 詳細なログや中間データはサブエージェントのコンテキストに任せる

### サブエージェント活用パターン

| パターン | 説明 | 効果 |
|----------|------|------|
| 分散リサーチ | 各トピックを別エージェントに割り当て | コンテキストが分割される |
| 並列分析 | 独立したファイルを並行で処理 | 時間短縮＋コンテキスト軽量化 |
| 段階的生成 | 大きなドキュメントをセクション単位で生成 | 1エージェントの負荷を分散 |
...
result:
  summary: "任務完了"
  sub_agents_used: 2  # 使用したサブエージェント数
